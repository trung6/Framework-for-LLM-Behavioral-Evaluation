...
while len(profiles_list) < n_profiles:
    ...
    genders = np.random.choice(gender_distribution, size=batch_size)
    educations = np.random.choice(education_levels, size=batch_size)
    maritals = np.random.choice(marital_status, size=batch_size)
    locations = np.random.choice(location_type, size=batch_size)
    ages = np.random.choice(age_groups, size=batch_size)
